...
react-router-dom, react-dom (navigation)
react-icons (icons and fonts)
bootstrap (Card/Grid styling)
ag-grid-community (Grid styling)
====
...
====
run npm start 
url for output is "https:/localhost/3000"
====
...
- To paginate results use the `_page` parameter, eg: `/products?_page=10&_limit=15` (returns 15 results starting from the 10th page).
- To sort results use the `_sort` parameter, eg: `/products?_sort=price`. Valid sort values are `price`, `size` and `id`.
